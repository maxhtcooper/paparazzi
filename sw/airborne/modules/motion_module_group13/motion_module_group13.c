...
static void optic_flow_cb(uint8_t __attribute__((unused)) sender_id,
                               ...
                               int16_t  __attribute__((unused)), int16_t  __attribute__((unused)),
...
void motion_module_group13_periodic(void)
{
  ...
  if (obstacle_detected) {
    ...
    float turn_base_deg = 10.f;
    float turn_gain_deg = 0.05;

    turn_increment = turn_base_deg + turn_gain_deg * avg_flow;
  ...
  VERBOSE_PRINT("avg_flow: %ld state: %d detection: %d turning: %d\n", (long)avg_flow, navigation_state, obstacle_detected, turn_increment);

  // bound obstacle_free_confidence
  Bound(obstacle_free_confidence, 0, max_trajectory_confidence);

  float moveDistance = obstacle_detected ? oa_min_forward_distance : maxDistance;
  ...
  //   moveDistance = oa_min_forward_distance;
  // } else {
  //   // No obstacle detected (low avg edge flow) but corridor is not open
  ...
  switch (navigation_state){
    ...
    //   break;
    case OUT_OF_BOUNDS:
      increase_nav_heading(oob_hdg_incr_deg);
